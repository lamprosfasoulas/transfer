...
func (f *Filesystem) GetUserSpace(c context.Context, u string) (int64, error) {
	_, size, err := f.ListFiles(c, u)
	return size, err
}

func (f *Filesystem) ListFiles(c context.Context, u string) ([]FileInfo, int64, error) {
	return nil, 0, nil
}
